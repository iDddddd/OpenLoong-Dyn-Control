...
int main(int argc, const char** argv)
{
    ...
    PVT_Ctr pvtCtr(mj_model->opt.timestep,"../common/g1_joint_ctrl_config.json");// PVT joint control
    ...
    RobotState.width_hips = 0.1289; // G1 髋部宽度
    ...
    while( !glfwWindowShouldClose(uiController.window))
    {
        ...
        while( mj_data->time - simstart < 1.0/60.0 && uiController.runSim) // press "1" to pause and resume, "2" to step the simulation
        {
            ...
            if (simTime<=startSteppingTime){
                ...
                // 直接使用resLeg.jointPosRes的所有值，不修改任何部分
